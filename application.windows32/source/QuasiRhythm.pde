...
void setup() {
  ...
  final Gain g = new Gain(ac, 1, new Envelope(ac, 0.05));
  final Gain m = new Gain(ac, 1, new Envelope(ac, 0.05));
  final Gain m2 = new Gain(ac, 1, new Envelope(ac, 0.05));
  ...
  final Bead nextInterval = new Bead() {
    public void messageReceived(Bead message) {
      ...
      if (distx <= disty && distx <= distz) {
        ...
        env.addInput(bass);
        ac.out.addInput(env);
        ((Envelope)env.getGainEnvelope()).addSegment(0.5,10);
      ...
      ((Envelope)g.getGainEnvelope()).addSegment(0.05, 1);
  ...
  final Bead melody_nextInterval = new Bead() {
    public void messageReceived(Bead message) {
      ...
      if (random(0,1) > skip_probability) {
        ...
        h.addInput(wp);
        ac.out.addInput(h);
        ((Envelope)h.getGainEnvelope()).addSegment(0.08, 10);
      ...
      //println(intervalLength);
      ((Envelope)m.getGainEnvelope()).addSegment(0.05, 1);
  ...
  final Bead melody2_nextInterval = new Bead() {
    public void messageReceived(Bead message) {
      ...
      //println(intervalLength);
      ((Envelope)m2.getGainEnvelope()).addSegment(0.05, 1);
  ...
  ((Envelope)m.getGainEnvelope()).addSegment(0.05,100,melody_nextInterval);
  ((Envelope)m2.getGainEnvelope()).addSegment(0.05,100,melody2_nextInterval);
  g.addInput(wp);
  ac.out.addInput(m);
  ac.out.addInput(m2);
  ac.out.addInput(g);
